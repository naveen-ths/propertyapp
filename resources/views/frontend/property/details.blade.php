...
<head>
  ...
  <!-- Title -->
  <title>{{ $propertyDetails->property_title ?? 'Property Details' }}</title>
  
  <!-- Favicons -->
  <link rel="icon" type="image/x-icon" href="{{ asset('assets/img/property/propertyfavicon/'.$propertyDetails->property_favicon) }}">
  <link rel="icon" href="{{ asset('assets/img/favicon.png') }}" sizes="16x16">
  <link rel="apple-touch-icon" href="{{ asset('assets/img/property/propertyfavicon/'.$propertyDetails->property_favicon) }}">

  <!-- SEO Meta Tags -->
  <meta name="description" content="{{ $propertyDetails->meta_description ?? 'Premium property details and information' }}">
  <meta name="keywords" content="{{ $propertyDetails->meta_keywords ?? 'property, real estate, homes, apartments' }}">
  <meta name="author" content="{{ $propertyDetails->property_title ?? config('app.name') }}">
  <meta name="robots" content="index, follow">
  <meta name="theme-color" content="{{ $propertyDetails->property_color ?? '#007bff' }}">
  ...
  <meta property="og:title" content="{{ $propertyDetails->property_title ?? 'Property Details' }}" />
  <meta property="og:description" content="{{ $propertyDetails->meta_description ?? 'Premium property details and information' }}" />
  <meta property="og:image" content="{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo) }}" />
  ...
  <meta name="twitter:title" content="{{ $propertyDetails->property_title ?? 'Property Details' }}" />
  <meta name="twitter:description" content="{{ $propertyDetails->meta_description ?? 'Premium property details and information' }}" />
  <meta name="twitter:image" content="{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo) }}" />

  <!-- Additional Social Meta Tags -->
  @if(!empty($propertyDetails->social_meta_tags))
    {!! $propertyDetails->social_meta_tags !!}
  @endif

  <style>
    :root {
      --colorPrimary: {{ $propertyDetails->property_color ?? '#007bff' }};
      --colorSecondary: {{ $propertyDetails->property_color ?? '#007bff' }};
      --colorBtn: #ffffff
    }
  ...
  <link rel="preload" href="{{ asset('assets/pages/fonts/roboto-bold-webfont.woff2') }}" as="font" type="font/woff2"
    crossorigin>
  <link rel="preload" href="{{ asset('assets/pages/fonts/roboto-regular-webfont.woff2') }}" as="font" type="font/woff2"
    crossorigin>
  <link rel="preload" href="{{ asset('assets/pages/fonts/muli-variablefont_wght-webfont.woff2') }}" as="font"
  ...
  <script type="application/ld+json">
  {
    ...
    "name": "{{ $propertyDetails->property_title ?? 'Property Details' }}",
    "description": "{{ $propertyDetails->meta_description ?? 'Premium property details and information' }}",
    "url": "{{ url()->current() }}",
    "image": "{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo) }}",
    "address": {
      "@type": "PostalAddress",
      "addressLocality": "{{ $propertyDetails->property_location ?? '' }}",
      "addressCountry": "IN"
    },
    "offers": {
      "@type": "Offer",
      "price": "{{ $propertyDetails->property_price ?? '' }}",
      "priceCurrency": "INR"
    },
    "builder": {
      "@type": "Organization",
      "name": "{{ $propertyDetails->builder_name ?? '' }}"
    }
  }
  </script>
...
<body data-spy="scroll" data-target="#navbarNav">
  ...
  <div id="loader">
    <span class="loader-text"><img
        src="{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo); }}"
  ...
  <style>
    ...
    .offer-bg-animation {
      ...
      background: {{ $propertyDetails->property_color ?? '#007bff' }};
  ...
  <header class="micro-nav fixed-top pload">
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white micro-navbar">
      <a class="navbar-brand" href="#" onclick="return false">
        <img src="{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo); }}"
          alt="{{$propertyDetails->property_title}}" class="logo">
      </a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav nav-fill">
          ...
          <li class="nav-item"><a class="nav-link" href="#sitefloorplan"><i class="mi mi-siteplan nav-icon"></i> Site
              &amp; Floor Plan</a></li>
          <li class="nav-item"><a class="nav-link" href="#amenities"><i class="mi mi-ami nav-icon"></i> Amenities</a>
          </li>
          <li class="nav-item"><a class="nav-link" href="#gallery"><i class="mi mi-gallery nav-icon"></i> Gallery</a>
          </li>
          <li class="nav-item"><a class="nav-link" href="#location"><i class="mi mi-location nav-icon"></i> Location</a>
          </li>
          <li class="nav-item"><a class="nav-link" href="#sitevisit"><i class="mi mi-sitevisit nav-icon"></i> Virtual
              Site Tour</a></li>
          <li class="nav-item overflow-hidden"><a class="nav-link enqModal" href="#" data-form="md"
              ...
              data-redirect="brochure" data-redirect-file="brochure.pdf" data-toggle="modal" data-target="#enqModal"><i
                class="mi mi-download nav-icon d-inline-block animated slideInDown infinite"></i> Brochure</a></li>
        </ul>
      </div>
    </nav>
  </header>
  <main class="pload">
    <div id="home" class="carousel slide micro-main-slider" data-ride="carousel">
        ...
        <div class="carousel-item">
          <picture>
            <source class="lazyload d-block micro-main-slider-img" media="(max-width: 750px)"
              data-srcset="{{ asset('assets/img/property/banners/'.$slide->image); }}"
              alt="Hubtown 25 Downtown in Tardeo, South Mumbai, Get 4 BHK & 5 BHK - Price, Location" type="image/webp">
            ...
            <img data-sizes="auto" class="lazyload d-block micro-main-slider-img"
              data-srcset="{{ asset('assets/img/property/banners/'.$slide->image); }} 750w,{{ asset('assets/img/property/banners/'.$slide->image); }}">
          </picture>
        </div>
        @empty
        <tr>
      ...
      <div class="info-box overflow-hidden">
        ...
        <br>
        @if(!empty($propertyDetails->banner_content))
        ...
        <span class="pro-rera"><span class="heading">Maha RERA Regd. Number.</span> : {{
          $propertyDetails->property_rera_no }} !
        </span>
      </div>
      <div class="mob-form d-sm-block d-md-none d-lg-none">
        ...
        <form action="{{ route('enquiry.submit') }}" method="POST" class="form-side">
          ...
          <div class="form-group">
            <input type="hidden" name="subject" value="Get Instant Call Back" class="form-control mb-3" id=""
              placeholder="Name">
            <input type="tel" id="intTelInput-mob" pattern="[0-9]+" name="mobile"
          ...
          <button type="submit" name="enq_formBtn" class="btn btn-info micro-form-btn effetMoveGradient"
            onclick="setCookie('redirectCookie', 'enquire');">Pre-Register Now</button>
        </form>
        <!--more button start-->
        <div class="mobile_button">
          @if(!empty($propertyDetails->sidebar_content))
      ...
      <section class="section shadow-sm lazyload" id="overview">
        ...
        <span class="d-block section-heading color-primary text-capitalize">{{ $propertyDetails->property_title }}
        </span>
        <span class="d-block section-heading-sub color-primary text-capitalize">At {{
          $propertyDetails->property_short_desc }}</span>
        @if(!empty($propertyDetails->property_desc))
        ...
        <button class="btn btn-sm btn-outline-info sectio-bro-btn overflow-hidden enqModal" data-form="md"
          ...
          data-redirect-file="brochure.pdf" data-toggle="modal" data-target="#enqModal"><span
            class="d-inline-block mi mi-download mr-1 animated slideInDown infinite"></span> Download Brochure</button>
        ...
        <div class="row">
          <div class="col-sm-12">
            @if(!empty($propertyDetails->property_highlights))
        ...
        <iframe width="100%" height="315" src="{{ $propertyDetails->property_video_url }}" title="YouTube video player"
          frameborder="0"
                  ...
                  @foreach($floors as $floor)
                    <tr>
                      ...
                      <td class="border border-left-0 border-top-0 border-bottom-0 price-carpet">
                        {{ $floor['carpet_area'] ?? '' }}<small class="d-inline-block d-md-none">(Carpet Area)</small>
                      </td>
                      <td class="price-amt"><i class="mi mi-rs-light"></i>
                        {{ $floor['flat_price'] ?? '' }}
                      </td>
                      <td><button class="btn btn-sm btn-info effetGradient effectScale enqModal" data-form="lg"
                          ...
                          data-redirect="enquiry" data-toggle="modal" data-target="#enqModal">Click Here</button></td>
                    </tr>
                  @endforeach
                @endif
              </tbody>
                ...
                <div class="at-property-img">
                  <picture>
                    <source class="lazyload w-100"
                      data-srcset="{{ asset('assets/img/property/costimage/'.$propertyDetails->complete_costing_details); }}"
                      alt="Floor Plan, Brochure, sample flat, Project reviews, complaints, Near Station, Possession Date, Payment Plan, Cost Sheet"
                      type="image/webp">
      ...
      <section class="section shadow-sm lazyload"
        data-script="{{ asset('assets/pages/plugins/fancybox/jquery.fancybox.min.js') }}"
            ...
            <a href="#" class="text-decoration-none enqModal" data-form="lg" data-title="Send me plan details"
              ...
              <div class="at-property-item mt-1">
                <div class="at-property-img master-plan text-center">
                  <picture>
                    <source class="lazyload shadow-sm border border-grey blur"
                      data-srcset="{{ asset('assets/img/property/masterplanimage/'.$propertyDetails->master_plan_image); }}"
                      alt="Floor Plan, Brochure, sample flat, Project reviews, complaints, Near Station, Possession Date, Payment Plan, Cost Sheet"
                      type="image/webp">
                    <img data-sizes="auto" class="lazyload shadow-sm border border-grey blur"
                      data-srcset="{{ asset('assets/img/property/masterplanimage/'.$propertyDetails->master_plan_image); }}"
                      alt="Floor Plan, Brochure, sample flat, Project reviews, complaints, Near Station, Possession Date, Payment Plan, Cost Sheet"
              ...
              @if(!empty($floors))
                ...
                <a href="#" class="text-decoration-none enqModal" data-form="lg" data-title="Send me plan details"
                  ...
                  <div class="at-property-item shadow-sm border border-grey mt-1">
                    <div class="at-property-img">
                      <picture>
                        <source class="lazyload floor-plan-img blur"
                          data-srcset="{{ asset('assets/img/property/floorimage/'.$floor->image); }}" type="image/webp">
                        <img data-sizes="auto" class="lazyload floor-plan-img blur"
                          data-srcset="{{ asset('assets/img/property/floorimage/'.$floor->image); }}"
                          alt="Floor Plan, Brochure, sample flat, Project reviews, complaints, Near Station, Possession Date, Payment Plan, Cost Sheet">
                      </picture>
                      <div class="at-property-overlayer"></div>
      ...
      <section class="section shadow-sm lazyload">
        ...
        <div class="row">
          ...
          <div class="col-lg-3 col-md-4 col-sm-6 col-6 mb-2">
            <a data-fancybox="gallery-0" href="{{ asset('assets/img/property/gallery/'.$g->image); }}"
              alt="{{ $propertyDetails->property_title }}">
              <img data-src="{{ asset('assets/img/property/gallery/'.$g->image); }}"
                alt="{{ $propertyDetails->property_title }}" class="lazyload gallery-thumb">
            </a>
          </div>
          @empty
          @endforelse
      ...
      <section class="section shadow-sm lazyload"
        ...
        <div class="row mb-3">
          <div class=" col-sm-12">
            ...
            <iframe class="lazyload lmap mb-2" src="{{ $propertyDetails->map_embed_url }}" allowfullscreen=""
        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3">
          @foreach($locationTextArray as $key => $l)
            <div class="col my-2"><i class="mi mi-loc-list-2 color-primary loc-icon"></i>
              {{ $l }}
            </div>
          @endforeach
        </div>
        ...
        <a href="#" class="enqModal" data-form="lg" data-title="Virtual Site Visit" data-btn="Start Tour"
          ...
          <div class="at-property-item my-2 pt-md-0">
            <div class="at-property-img vsv-img">
              <picture>
                <source class="lazyload w-100" media="(max-width: 750px)"
                  data-srcset="{{ asset('assets/img/property/virtualsitetour/'.$propertyDetails->virtual_site_tour) }}"
                  type="image/webp">
                <source class="lazyload w-100" media="(min-width: 751px)"
                  data-srcset="{{ asset('assets/img/property/virtualsitetour/'.$propertyDetails->virtual_site_tour) }}"
                  type="image/webp">
                <img data-sizes="auto" class="lazyload w-100"
                  data-srcset="{{ asset('assets/img/property/virtualsitetour/'.$propertyDetails->virtual_site_tour) }} 750w,{{ asset('assets/img/property/virtualsitetour/'.$propertyDetails->virtual_site_tour) }}">
              </picture>
              <div class="vsv-text-bk">
                <div class="vsv-text-bg">
                  ...
                  <span class="text-capitalize text-center d-none d-lg-block">{{ $propertyDetails->property_title }} in
                    {{ $propertyDetails->property_location }}</span>
                </div>
              </div>
            </div>
            <span class="d-block text-capitalize text-center d-sm-block d-md-none pt-2">{{
              $propertyDetails->property_title }} in {{ $propertyDetails->property_location }}</span>
          </div>
        </a>
      </section>
      <section class="section shadow-sm lazyload" id="developer">
        <div class="d-block pt-2 pb-1 text-center">
          <img src="{{ asset('assets/img/property/propertylogo/'.$propertyDetails->property_logo) }}"
            style="max-width: 100%;height: 70px; display: inline-block;">
        </div>
        <span class="d-block section-heading-sub text-capitalize">{{ $propertyDetails->builder_name }}</span>
        @if(!empty($propertyDetails->about_developer))
          {!! $propertyDetails->about_developer !!}
        @endif

      </section>
      <section class="section shadow-sm lazyload">
        @if(!empty($propertyDetails->property_footer_content))
          {!! $propertyDetails->property_footer_content !!}
        @endif

      </section>
      <p style="text-align:center;">Copyright &copy;
        {{ date('Y') }} | <a href="/">Terms & Conditions</a> | <a href="/">Privacy
          Policy</a> | <a href="/">Cookies Policy</a>
      </p>
      <div class="micro-side text-center">
        <div class="og-section pb-2">
          <ul class="nav nav-fill og-block">
            <li class="nav-item enqModal" data-form="lg" data-title="Organize Site Visit" data-btn="Request Site Visit"
              data-enquiry="Organize Site Visit" data-toggle="modal" data-target="#enqModal">Organize Site Visit</li>
            <li class="nav-item"
              onclick="window.open('https://api.whatsapp.com/send?phone=+91{{ $propertyDetails->mobile_no }}&text=I want More Information About Hubtown 25 Downtown Tardeo, South Mumbai Project.', '_blank');">
              <span class="mi mi-whatsapp action-icon"><span class="path1"></span><span class="path2"></span><span
                  class="path3"></span></span> +91 {{ $propertyDetails->mobile_no }}
            </li>
          </ul>
        ...
        <form action="{{ route('enquiry.submit') }}" method="POST" class="form-side">
          ...
          <div class="form-group">
            <input type="hidden" name="subject" value="Get Instant Call Back" class="form-control mb-3" id=""
              placeholder="Name">
            <input type="tel" id="intTelInput-side" pattern="[0-9]+" name="mobile"
              class="intTelInput form-control rounded-0 micro-form-field w-100" placeholder="Mobile No" required>
          </div>
          <div class="form-group">
            <input type="email" name="email" class="form-control rounded-0 micro-form-field"
      ...
      <ul class="mob-action nav nav-fill d-sm-block d-lg-none">
        <li class="nav-item" onclick="javascript:location.href = 'tel:+91{{ $propertyDetails->mobile_no }}'"><span
                  ...
                  <form method="POST" action="{{ route('enquiry.submit') }}" class="form-modal">
                    ...
                    <div class="form-group">
                      <input type="text" name="name" pattern="[a-zA-Z ]+"
                        class="form-control rounded-0 micro-form-field" required placeholder="Name">
                    </div>
                    <input type="hidden" name="subject" value="Get Instant Call Back" class="form-control mb-3" id=""
                      placeholder="Name">
                    <input type="hidden" name="property_url" value="{{ url()->current() }}" />
                    <div class="form-group">
                      <input type="tel" id="intTelInput-popup" pattern="[0-9]+" name="mobile"
                        class="intTelInput form-control rounded-0 micro-form-field" required placeholder="Mobile No">
                    </div>
                    ...
                    <button type="submit" class="btn btn-info micro-form-btn effetGradient effectScale">Get Instant Call
                      Back</button>
                  </form>
                </div>
              </div>
              <a href="tel:+91{{ $propertyDetails->mobile_no }}" class="modal-call-btn"><i class="mi mi-call"></i> +91
                {{ $propertyDetails->mobile_no }}</a>
            </div>
          </div>
        </div>
      </div>
